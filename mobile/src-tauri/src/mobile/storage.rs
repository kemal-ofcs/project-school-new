...
pub fn initialize(path: &Path) -> Result<(), String> {
    ...
    connection
      ...
      INSERT OR IGNORE INTO desktop_schema_migration (version, name, applied_at)
      VALUES (5, 'mobile-academic-foundation', unixepoch());
      INSERT OR IGNORE INTO desktop_schema_migration (version, name, applied_at)
      VALUES (6, 'mobile-academic-unique-relaxation', unixepoch());
      ...
      --
      -- `storage.rs` TIDAK ikut `sync-rust-modules.ts` (Mobile memakai nama
      -- database dan nama migrasi sendiri), jadi DDL ini disunting tangan di
      -- KEDUA workspace. `audit:schema` bagian 3 yang menjaga keduanya tetap
      -- sama — dan memang itu yang menangkap ketiadaan blok ini.
      --
      ...
      INSERT OR IGNORE INTO desktop_schema_migration (version, name, applied_at)
      VALUES (7, 'mobile-class-attendance-foundation', unixepoch());
      ...
      INSERT OR IGNORE INTO desktop_schema_migration (version, name, applied_at)
      VALUES (8, 'mobile-teaching-journal-and-attendance-ledger', unixepoch());
      ...
      INSERT OR IGNORE INTO desktop_schema_migration (version, name, applied_at)
      VALUES (9, 'mobile-whatsapp-notification-queue', unixepoch());
      ...
      -- tarif umum. SENGAJA tanpa UNIQUE selain PK: dua perangkat offline boleh
      -- membuat tarif untuk mapel yang sama, dan sebuah UNIQUE akan membuat
      -- push-nya gagal permanen. Cerminan DDL di storage.rs Desktop.
      ...
      INSERT OR IGNORE INTO desktop_schema_migration (version, name, applied_at)
      VALUES (10, 'mobile-teaching-jp-rate', unixepoch());
      -- Jadwal bel sekolah (schema versi 23).
      --
      -- Tabel KETERANGAN: presensi kelas tetap menyimpan `jam_ke` dan honor
      -- tetap dihitung per jam pelajaran, jadi baris yang belum lengkap tidak
      -- pernah menghalangi presensi. SENGAJA tanpa UNIQUE pada `jam_ke`: dua
      -- perangkat offline boleh mendaftarkan jam yang sama, dan UNIQUE akan
      -- membuat push-nya gagal permanen. Cerminan DDL di storage.rs Desktop.
      ...
      INSERT OR IGNORE INTO desktop_schema_migration (version, name, applied_at)
      VALUES (11, 'mobile-lesson-period-schedule', unixepoch());
      ...
      -- `hari` disimpan 1=Senin sampai 7=Minggu. SENGAJA tanpa UNIQUE — dua
      -- perangkat offline boleh menyusun jadwal yang sama, dan UNIQUE akan
      -- membuat push-nya gagal permanen. Cerminan DDL di storage.rs Desktop.
      ...
      INSERT OR IGNORE INTO desktop_schema_migration (version, name, applied_at)
      VALUES (12, 'mobile-teaching-schedule', unixepoch());
      "#,
        )
