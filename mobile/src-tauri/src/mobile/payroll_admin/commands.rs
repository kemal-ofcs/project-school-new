...
// Sunting sumbernya: `web-desktop/src-tauri/src/desktop/payroll/commands.rs`.
...
fn load_taught_sessions(
    ...
    for item in rows.flatten() {
        ...
        let Some((awal, akhir)) = crate::mobile::class_attendance::jam_ke_range(&jam_ke) else {
            continue;
        };
        per_guru
            .entry(id_guru)
...
#[tauri::command]
pub async fn desktop_get_jp_rates(state: State<'_, MobileState>) -> Result<Value, CommandError> {
    require_permission(&state, "payroll.view")?;
    let conn = storage::database(&state.data_dir)?;
