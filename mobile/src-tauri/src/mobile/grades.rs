...
// Sunting sumbernya: `web-desktop/src-tauri/src/desktop/grades.rs`.
...
pub fn get_assessment(state: &MobileState, id_penilaian: &str) -> Result<Value, CommandError> {
    let conn = storage::database(&state.data_dir)?;
...
pub fn save_assessment(state: &MobileState, draft: &Value) -> Result<Value, CommandError> {
...
pub fn save_scores(state: &MobileState, draft: &Value) -> Result<Value, CommandError> {
    let mut conn = storage::database(&state.data_dir)?;
    let tx = conn.transaction().map_err(|_| CommandError::internal())?;
...
pub fn delete_assessment(state: &MobileState, id_penilaian: &str) -> Result<Value, CommandError> {
    let mut conn = storage::database(&state.data_dir)?;
    let tx = conn.transaction().map_err(|_| CommandError::internal())?;
