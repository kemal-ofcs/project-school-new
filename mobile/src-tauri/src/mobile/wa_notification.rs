...
pub fn get_wa_templates(state: &MobileState) -> Result<Value, CommandError> {
    let connection = storage::database(&state.data_dir)?;
...
pub fn save_wa_templates(state: &MobileState, templates: &Value) -> Result<Value, CommandError> {
