...
pub fn list_lesson_periods(state: &MobileState) -> Result<Value, CommandError> {
...
pub fn save_lesson_period(state: &MobileState, draft: &Value) -> Result<Value, CommandError> {
...
pub fn delete_lesson_period(state: &MobileState, id: &str) -> Result<Value, CommandError> {
    let mut conn = storage::database(&state.data_dir)?;
    let tx = conn.transaction().map_err(|_| CommandError::internal())?;
...
pub fn get_jp_settings(state: &MobileState) -> Result<Value, CommandError> {
    let conn = storage::database(&state.data_dir)?;
