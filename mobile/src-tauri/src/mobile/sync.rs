...
// Sunting sumbernya: `web-desktop/src-tauri/src/desktop/sync.rs`.
...
fn load_tombstone_cursor(state: &MobileState) -> Result<i64, CommandError> {
    let connection = storage::database(&state.data_dir)?;
    Ok(connection
        .query_row(
...
mod tests {
    ...
    use super::{
        ...
        pending_events, prune_settled_outbox, storage, synchronize, MobileState, SnapshotTable,
    ...
    fn employee_ids(state: &MobileState) -> Vec<(String, Option<String>)> {
        let connection = storage::database(&state.data_dir).expect("local database");
        let mut statement = connection
