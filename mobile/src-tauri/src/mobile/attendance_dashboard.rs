...
// Sunting sumbernya: `web-desktop/src-tauri/src/desktop/attendance_dashboard.rs`.

use rusqlite::params;
use serde_json::{json, Value};

use super::{config::MobileState, models::CommandError, storage};
...
mod tests {
    ...
    fn setup_test_state() -> (tempfile::TempDir, MobileState) {
    ...
    fn seed_data_uji(state: &MobileState) {
        let connection = storage::database(&state.data_dir).expect("open db");
        connection
            .execute_batch(
                r#"
