...
// Sunting sumbernya: `web-desktop/src-tauri/src/desktop/academic.rs`.
...
pub fn list_academic_years(state: &MobileState) -> Result<Value, CommandError> {
...
pub fn save_academic_year(state: &MobileState, draft: &Value) -> Result<Value, CommandError> {
...
pub fn list_academic_units(state: &MobileState) -> Result<Value, CommandError> {
    let conn = storage::database(&state.data_dir)?;
    let mut stmt = conn
        .prepare(
            r#"
...
pub fn save_academic_unit(state: &MobileState, draft: &Value) -> Result<Value, CommandError> {
    let mut conn = storage::database(&state.data_dir)?;
    let tx = conn.transaction().map_err(|_| CommandError::internal())?;
...
pub fn delete_academic_unit(state: &MobileState, id: &str) -> Result<Value, CommandError> {
    let mut conn = storage::database(&state.data_dir)?;
    let tx = conn.transaction().map_err(|_| CommandError::internal())?;
...
pub fn list_academic_departments(state: &MobileState) -> Result<Value, CommandError> {
    let conn = storage::database(&state.data_dir)?;
    let mut stmt = conn
        .prepare(
            r#"
...
pub fn save_academic_class(state: &MobileState, draft: &Value) -> Result<Value, CommandError> {
    let mut conn = storage::database(&state.data_dir)?;
    let tx = conn.transaction().map_err(|_| CommandError::internal())?;
...
pub fn list_academic_subjects(state: &MobileState) -> Result<Value, CommandError> {
    let conn = storage::database(&state.data_dir)?;
    let mut stmt = conn
        .prepare(
            r#"
...
pub fn save_academic_subject(state: &MobileState, draft: &Value) -> Result<Value, CommandError> {
    let mut conn = storage::database(&state.data_dir)?;
    let tx = conn.transaction().map_err(|_| CommandError::internal())?;
...
pub fn save_teaching_schedule(
    ...
    // presensi disimpan.
    let jam_ke = crate::mobile::class_attendance::normalize_jam_ke_public(text(draft, "jam_ke"))?;
    ...
    if is_aktif == 1 {
        if let Some(bentrok) = tersimpan
            .iter()
            .find(|lain| crate::mobile::class_attendance::jam_ke_overlaps_public(&jam_ke, lain))
        {
            return Err(CommandError::new(
                "DUPLICATE_SCHEDULE",
                format!(
...
pub fn delete_teaching_schedule(state: &MobileState, id: &str) -> Result<Value, CommandError> {
    let mut conn = storage::database(&state.data_dir)?;
    let tx = conn.transaction().map_err(|_| CommandError::internal())?;
...
pub fn list_teachers(state: &MobileState) -> Result<Value, CommandError> {
    let conn = storage::database(&state.data_dir)?;
    let sql = r#"
...
pub fn save_teacher(state: &MobileState, draft: &Value) -> Result<Value, CommandError> {
    let mut conn = storage::database(&state.data_dir)?;
    let tx = conn.transaction().map_err(|_| CommandError::internal())?;
...
pub fn list_students(state: &MobileState, id_rombel: Option<&str>) -> Result<Value, CommandError> {
    let conn = storage::database(&state.data_dir)?;
    let sql = r#"
...
pub fn save_student(state: &MobileState, draft: &Value) -> Result<Value, CommandError> {
    let mut conn = storage::database(&state.data_dir)?;
    let tx = conn.transaction().map_err(|_| CommandError::internal())?;
...
pub fn get_personnel_photo(state: &MobileState, id_unik: &str) -> Result<Value, CommandError> {
    use rusqlite::OptionalExtension;
    let conn = storage::database(&state.data_dir)?;
    let mut stmt = conn
...
pub fn cache_personnel_photo_local(state: &MobileState, photo: &Value) -> Result<(), CommandError> {
...
pub fn delete_personnel_photo(state: &MobileState, id_unik: &str) -> Result<Value, CommandError> {
    let clean_id = id_unik.trim();
    if clean_id.is_empty() {
        return Err(CommandError::new(
            "VALIDATION_ERROR",
...
pub fn personnel_kind(state: &MobileState, id_unik: &str) -> Result<PersonnelKind, CommandError> {
    let conn = storage::database(&state.data_dir)?;
...
pub fn has_pending_photo_change(state: &MobileState, id_unik: &str) -> Result<bool, CommandError> {
    let conn = storage::database(&state.data_dir)?;
    conn.query_row(
...
pub fn drop_personnel_photo_cache(state: &MobileState, id_unik: &str) -> Result<(), CommandError> {
    let conn = storage::database(&state.data_dir)?;
    conn.execute(
        "DELETE FROM personil_foto WHERE id_unik = ?1;",
        params![id_unik.trim()],
...
mod tests {
    ...
    fn shift_of(state: &MobileState, id: &str) -> i64 {
        storage::database(&state.data_dir)
            .expect("database lokal")
            .query_row(
    ...
    fn kode_of(state: &MobileState, id: &str) -> String {
        storage::database(&state.data_dir)
            .expect("database lokal")
            .query_row(
